...
import {Role, RoleCreateRequest} from '../models';
...
export class RoleController {
  ...
  async create(
    ...
    if (!profile.permissions.includes(PERMISSIONS.WRITE_ROLES))
  ...
  async count(
    ...
    const profile = JSON.parse(currentUserProfile[securityId]);
    if (!profile.permissions.includes(PERMISSIONS.READ_ROLES))
      throw new HttpErrors.Unauthorized(MESSAGES.NO_PERMISSION)
