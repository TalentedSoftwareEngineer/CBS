...
import {get, oas, post, HttpErrors, param, response, Response, RestBindings} from '@loopback/rest';
...
export class CdrLogController {
  ...
  async nap(
    ...
    try {
      return await this.cdrService.getNAPs()
    } catch (err) {
      return []
    }
  }
  ...
  async count(
      @inject(SecurityBindings.USER) currentUserProfile: UserProfile,
      @param.query.string('server') server: string,
    ...
    try {
      const payload = {
        server, value, start_at, end_at, calls_op, calls, duration_op, duration, nap
      }
  ...
  async find(
    ...
    try {
      const payload = {
        server, limit, skip, order, value, start_at, end_at, calls_op, calls, duration_op, duration, nap
      }
  ...
  async export(
    ...
    try {
      const payload = {
        server, order, value, start_at, end_at, calls_op, calls, duration_op, duration, nap
      }

      const result = await this.cdrService.export(payload)
      return result
    } catch (err) {
      console.log(err)
      throw new HttpErrors.BadRequest(MESSAGES.NO_CDR_LOG)
