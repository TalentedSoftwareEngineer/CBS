...
export class ApiService {
  ...
  getStatements(active: string, direction: string, page: number, size: number, filterValue: string, customer_id: any, server_id: any, start_at: any, end_at: any, status: any): Observable<any[]> {
    const parametersQuery = new URLSearchParams({
      limit: `${size}`,
      skip: `${(page - 1) * size}`,
      order: `${active} ${direction}`,
    ...
    const url = `${this.coreApi}/statement/find?${parametersQuery}`;
    return this.http.get<any[]>(url);
  }

  getStatementCount(filterValue: string, customer_id: any, server_id: any, start_at: any, end_at: any, status: any): Observable<any> {
    const parametersQuery = new URLSearchParams({
      value: `${filterValue}`,
      customer_id: `${customer_id}`,
      server_id: `${server_id}`,
    ...
    return this.http.get<any>(`${this.coreApi}/statement/count?${parametersQuery}`);
  }
  ...
  checkStatement(
    ...
    const url = `${this.coreApi}/statement/check`;
  ...
  generateStatement(
    ...
    const body = {
      ...
      customer_id: Number(customer_id),
      server_id, start_at, end_at,
      total_calls, total_duration, total_cost,
      content: `${content}`
    }
    const url = `${this.coreApi}/statement/generate`;
    return this.http.post<any>(url, body);
  }
}
