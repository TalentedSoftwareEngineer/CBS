...
export class CreateBillingStatementComponent implements OnInit {
  ...
  isLoading = true
  isProcessing = false
  totalStatistics = { total_calls: '', total_minutes: '', avg_minutes: '', total_cost: '', avg_cost: ''}
  cdrDetails: any[] = []
  ...
  buildStatement = async (logs: any[]) => {
    ...
    for (let log of logs) {
      ...
      cdrDetails.push({
        ...log,
        StartTime: moment.utc(Number(log.StartTime) * 1000).utcOffset(-offset).format('MM/DD/YYYY h:mm:ss A'),
        EndTime: log.EndTime ? moment.utc(Number(log.EndTime) * 1000).utcOffset(-offset).format('MM/DD/YYYY h:mm:ss A') : '',
  ...
  onGenerate = () => {
    ...
    this.cdrDetails.forEach((item, index)=>{
      if(index % 20) {
        cdrDetailsTableBody += `
          <tr>
            <td style="border: 1px solid #000000;padding: 8px;">${item.StartTime}</td>
            <td style="border: 1px solid #000000;padding: 8px;">${item.EndTime}</td>
            <td style="border: 1px solid #000000;padding: 8px;">${item.Calling}</td>
            <td style="border: 1px solid #000000;padding: 8px;">${item.Called}</td>
            <td style="border: 1px solid #000000;padding: 8px;">${item.Duration}</td>
            <td style="border: 1px solid #000000;padding: 8px;">$${item.cost}</td>
          </tr>
        `;
      } else {
        cdrDetailsTableBody += `
          <tr style="background-color: #dddddd;">
            <th style="border: 1px solid #000000;padding: 8px;">Start Time</th>
            <th style="border: 1px solid #000000;padding: 8px;">End Time</th>
            <th style="border: 1px solid #000000;padding: 8px;">Calling</th>
            <th style="border: 1px solid #000000;padding: 8px;">Called</th>
            <th style="border: 1px solid #000000;padding: 8px;">Duration</th>
            <th style="border: 1px solid #000000;padding: 8px;">Cost</th>
          </tr>
          <tr>
            <td style="border: 1px solid #000000;padding: 8px;">${item.StartTime}</td>
    ...
    let html = `
      <div style="padding-top: 2rem;">
        ...
        <div style="width: 100%;display: flex; justify-content: space-between;">
          <h5 style="color: #000;margin-top: 1.5rem;">No.: 2023044439</h5>
