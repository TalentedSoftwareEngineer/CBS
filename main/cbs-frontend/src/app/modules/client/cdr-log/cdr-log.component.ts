  ...
  getCDRLogsList = async () => {
    ...
    try {
      ...
      await this.api.getCDRLogsList(
        ...
        this.cdr_logs = [];
        response.map(u => {
          // u.StartTime = u.StartTime ? moment.utc(Number(u.StartTime) * 1000).format(this.store.getUser()?.time_format ? this.store.getUser()?.time_format : 'MM/DD/YYYY h:mm:ss A') : '';
          // u.EndTime = u.EndTime ?  moment.utc(Number(u.EndTime) * 1000).format(this.store.getUser()?.time_format ? this.store.getUser()?.time_format : 'MM/DD/YYYY h:mm:ss A') : '';
          u.StartTime = u.StartTime ? moment.utc(Number(u.StartTime) * 1000).utcOffset(-offset).format(this.store.getUser()?.time_format ? this.store.getUser()?.time_format : 'MM/DD/YYYY h:mm:ss A') : '';
          u.EndTime = u.EndTime ?  moment.utc(Number(u.EndTime) * 1000).utcOffset(-offset).format(this.store.getUser()?.time_format ? this.store.getUser()?.time_format : 'MM/DD/YYYY h:mm:ss A') : '';
