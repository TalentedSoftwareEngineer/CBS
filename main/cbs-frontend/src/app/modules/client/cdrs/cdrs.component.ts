        ...
        .pipe(tap(async (response: any[]) => {
          ...
          response.map(u => {
            ...
            if(Boolean(this.store.getUser()?.timezone)) {
              ...
              u.start_date = u.start_date ? moment(u.start_date).utc().utcOffset(Number(this.store.getUser()?.timezone)).format(this.store.getUser()?.time_format ? this.store.getUser()?.time_format : 'MM/DD/YYYY h:mm:ss A') : '';
            } else {
              // Local time
              ...
              u.start_date = u.start_date ? moment(new Date(u.start_date)).format(this.store.getUser()?.time_format ? this.store.getUser()?.time_format : 'MM/DD/YYYY h:mm:ss A') : '';
            }
          });
